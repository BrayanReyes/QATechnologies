...
/**
 * Flights Result Page shows the result after perform a flights search, also
 * allows to sort the flights and select option for Departing and Returning
 * flight
 * 
 * @author july.moreno
 *
...
public class FlightsResultPage extends BasePage {
	...
	/**
	 * Validate if Flight Details and Baggage fees Label is present in each item
	 * result
	 *
	 * @return true: boolean
	 */
	...
	private int calculateTotalMinutesFlight(String flightDuration) {
		int durationInHourToMinutes = Integer
				.parseInt(flightDuration.substring(0, flightDuration.indexOf(HOUR_SEPARATOR))) * 60;
		int durationInMinutes = Integer.parseInt(
				flightDuration.substring(flightDuration.indexOf(" "), flightDuration.indexOf(MINUTE_SEPARATOR)).trim());
	...
	public void selectDepartingFLight(int departingOption) {
		...
		WebElement departureFlightSelected = flightsResultList.findElements(By.className(FLIGHT_MODULE_CLASS))
				.get(departingOption - 1);
	...
	public void selectReturningFLight(int returningOption) {
		...
		waitListToBeRefreshed(REFRESHED_FLIGHTS_LIST_CLASS);
		WebElement returnFlight = flightsResultList.findElements(By.className(FLIGHT_MODULE_CLASS))
				.get(returningOption - 1);
		returnFlight.findElement(By.className(SELECT_BUTTON_CLASS)).click();
		log.info("The user clicks \"Select\" button");
		getWait().until(ExpectedConditions
				.visibilityOf(returnFlight.findElement(By.cssSelector(RETURNING_FARE_BUTTON_ID + (returningOption)))));
		getWait().until(ExpectedConditions.elementToBeClickable(
				returnFlight.findElement(By.cssSelector(RETURNING_FARE_BUTTON_ID + (returningOption) + " button"))));
	...
	/**
	 * Continue with the package booking process and go to Summary Package Page
	 */
